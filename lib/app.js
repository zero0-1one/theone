...
module.exports = class {
  constructor(...args) {
    ...
    if (sessionCfg && sessionCfg['enabled']) {
      ...
      if (sessionCfg['signed']) {
        if (!(Array.isArray(sessionCfg['keys']) && sessionCfg['keys'].length > 0)) {
          throw new Error('If the session is configured as "signed:true", then "keys" must also be configured')
        }
        this.koa.keys = sessionCfg['keys']
      }
    }
  }
