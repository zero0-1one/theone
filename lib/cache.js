...
let fileCache = {
  ...
  key(name) {
    if (!this._keys.hasOwnProperty(name)) {
      ...
      sha1.update(name)
      this._keys[name] = sha1.digest('hex')
    }
    return this._keys[name]
  },

  init(isExpired) {
    if (!fs.existsSync(cacheOptions['dir'])) {
      fs.mkdirSync(cacheOptions['dir'])
    }
    for (let d of '0123456789abcdef') {
      let dirPath = path.join(cacheOptions['dir'], d)
      if (!fs.existsSync(dirPath)) {
        fs.mkdirSync(dirPath)
      }
    }
    this.gcSync(isExpired)
  },

  async clear(name) {
    let key = this.key(name)
    await fsPromises.unlink(path.join(cacheOptions['dir'], key[0], key + '.json'))
  },

  async get(name) {
    let key = this.key(name)
    let str = await fsPromises.readFile(path.join(cacheOptions['dir'], key[0], key + '.json'), 'utf8')
    return JSON.parse(str)
  },

  async set(name, data) {
    let key = this.key(name)
    await fsPromises.writeFile(path.join(cacheOptions['dir'], key[0], key + '.json'), JSON.stringify(data))
  },
...
function isExpired(data) { return data.expire != 0 && data.expire < Date.now() }

cache.init = function(options, rootDir = './', logger = function() {}, cacher = fileCache) {
  cache.close()
