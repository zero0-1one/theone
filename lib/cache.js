...
class Cache {
  constructor(options, version = 0) {
    ...
    if (options['gcSchedule']) {
      let times = options['gcCompleteTimes']
      if (times > 1) {
        let partIndex = 0
        this.gcJob = schedule.scheduleJob(options['gcSchedule'], () => {
          this.adapter.gc(this._isExpired.bind(this), [partIndex, times])
          partIndex++
          if (partIndex >= times) partIndex = 0
        })
      } else {
        this.gcJob = schedule.scheduleJob(options['gcSchedule'], () => {
          this.adapter.gc(this._isExpired.bind(this))
        })
      }
    }
  }
