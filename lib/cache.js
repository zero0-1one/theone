...
let cache = async function (name, value, timeout) {
  if (value === undefined) {
    ...
    return cache.remember(name, value, timeout)
  } else {
...
function isExpired(data) {
  return data.expire != 0 && data.expire * 1000 < Date.now()
}
...
cache.init = function (options, rootDir = './', logger = function () { }, cacher = fileCacher) {
  ...
  if (options['gcSchedule']) {
    cacheGcJob = schedule.scheduleJob(options['gcSchedule'], () => {
...
cache._data = function (value, timeout) {
  ...
  let expire = timeout == 0 ? 0 : Math.floor(Date.now() / 1000 + timeout) // 单位: s
  return { value, expire }
}

cache.remember = async function (name, valueFunc, timeout) {
  return toUtil.doOrder(name, cacheLocks, async () => {
    let data = await cacheCacher.get(name).catch(() => { })
    if (data !== undefined && !isExpired(data)) {
      return data.value
