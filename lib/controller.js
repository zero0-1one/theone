...
let controller = {
    ...
    return class extends parent {
      ...
      model(name) {
        if (!this[_models].hasOwnProperty(name)) {
          this[_models][name] = model.creatModle(this, name)
        }
  ...
  async callAction(ctx, moduleName, ctrl, action, args, common = {}) {
    let ctrlInst
    try {
      ...
      let call = async () => {
        ...
        let rt = await action.func.apply(ctrlInst, sortArgs)
  ...
  callAction() {
    ...
    return async function callAction(ctx, next) {
      ...
      args = Object.assign({}, ctx.query, args)
      let common = { version: version || args.version }
      let rt = await controller.callAction(ctx, moduleName, ctrl, action, args, common)
      rt.send(ctx)
      await next()
    }
  }
