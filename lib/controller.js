...
let controller = {
  extends(parent) {
    return class extends parent {
      ...
      sign(data, expiresIn) {
        ...
        return sign(data, cfg['secret'], options)
      }
  ...
  callAction(middlewares) {
    return async function callAction(ctx, next) {
      try {
        let { moduleName, ctrl, action, args, version } = ctx.pathInfo
