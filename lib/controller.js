...
let controller = {
    ...
    return class extends parent {
      ...
      async cache(name, value, timeout) {
        if (value !== undefined && value !== null) {
          let rootCtrl = this.ctx.rootCtrl
          rootCtrl[_caches] = rootCtrl[_caches] || new Set()
          rootCtrl[_caches].add(name)
        }
        return theone.cache(name, value, timeout)
      }

      //清空当前 action 设置的所有缓存， 通常用于 action 有异常需要回滚的时候
      async cacheRollback() {
        let rootCtrl = this.ctx.rootCtrl
        if (!rootCtrl[_caches]) return
        for (const name of rootCtrl[_caches]) {
          await theone.cache.clear(name)
        }
  ...
  getActionArgs(args, params) {
    ...
    for (let param of params) {
      ...
      if (arg === undefined) {
        if (param.conversion && !param.hasDefault)
          throw new RequestError(`参数 ${param.name} 必须是 ${param.type} 类型`)
      } else {
        if (param.conversion) {
      ...
      switch (paramAst.type) {
        ...
        case 'AssignmentPattern':
          if (paramAst.left.type == 'Identifier') {
            ...
            param.hasDefault = !(
              paramAst.right.type == 'Identifier' && toUtil.hasOwnPropertySafe(typeConversion, param.type)
            )
            if (toUtil.hasOwnPropertySafe(typeConversion, param.type)) {
  ...
  async _callAction(ctx, middlewares, moduleName, ctrl, action, args) {
    ...
    try {
      ...
      if (allModules[moduleName]._autoTransaction) {
        let rt = await ctrlInst[_mainDb].transaction(call).catch(e => {
      ...
      try {
        ...
        if (module._multiVersion) {
          if (toUtil.hasOwnPropertySafe(module, names[0])) {
            ...
            toUtil.hasOwnPropertySafe(ctx.query, 'version') &&
            toUtil.hasOwnPropertySafe(module, ctx.query.version)
          ) {
            version = ctx.query.version
          } else {
            throw new RequestError(
              `"${moduleName}" is multi-version module, but cannot find a valid version in the path`
            )
          }
          module = module[version]
        }
        ...
        while (names.length >= 2) {
          ...
          ctrlName = names.join('/')
          if (
            toUtil.hasOwnPropertySafe(module, ctrlName) &&
            toUtil.hasOwnPropertySafe(module[ctrlName].actions, actionName)
          ) {
