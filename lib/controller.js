...
let controller = {
  extends(parent) {
    return class extends parent {
      ...
      //不会会滚数据库
      failed(msg, data) {
        return new CallFailed(msg, data)
      }
      ...
      //回滚数据库
      error(status, msg) {
        throw new CallError(status, msg)
      }

      isFailed(rt) {
        return rt instanceof CallFailed
      }

      isError(rt) {
        return rt instanceof CallError
      }

      isSucceed(rt) {
        return !this.isFailed(rt) && !this.isError(rt)
      }
  ...
  async callAction(ctx, actionMiddlewares, moduleName, ctrl, action, args, common = {}) {
    let ctrlInst
    try {
      ctrlInst = new ctrl.class(ctx, moduleName, ctrl.name, common)
      ctrlInst[_init]()
      let call = async () => {
        let sortArgs = []
      ...
      let rt = await (allModules[moduleName]._autoTransaction ? ctrlInst[_mainDb].transaction(call) : call())
      if (ctrlInst.isSucceed(rt)) {
        rt = { status: 1, data: rt }
      }
      return rt
    } catch (e) {
      ...
      } else if (ctrlInst && ctrlInst.isError(e)) {
        theone.log.error('Call action exception,  actionPath:"%s", status:"%d" msg:"%s"', actionPath, e.status, e.msg)
        return e
      } else {
        theone.log.error('Call action exception,  actionPath:"%s", msg:"%s"', actionPath, e.toString())
      }
      return new CallError(-1, `Call action exception,  actionPath:"${actionPath}"`) //不对外暴露异常细节,记录日志
    } finally {
  ...
  async batchCall(ctx, actionMiddlewares, actions, common = {}) {
    ...
    for (let [moduleName, actionPath, args] of actions) {
      try {
        ...
        results.push(new CallError(-1, e.toString()))
      }
    }
    return results
  },
  ...
  parsePath(path) {
    ...
    return [moduleName, ctrl, ctrl.actions[actionName], args, version]
  },
  ...
  callActions(actionMiddlewares) {
    ...
    return async function callActions(ctx, next) {
      ...
      if (Array.isArray(actions) && actions.length > 0) {
        ...
        try {
          let [moduleName, ctrl, action, args, version] = controller.parsePath(ctx.path)
          common = common || {}
          ...
          ctx.body = new CallError(-1, e.toString())
        }
      }
      await next()
    }
