...
let controller = {
  ...
  async _callAction(ctx, middlewares, moduleName, ctrl, action, args, common = {}) {
    let ctrlInst
    try {
      ...
      let call = async () => {
        ...
        let rt = null
        if (middlewares) {
          rt = await middlewares(ctrlInst, async () => {
  ...
  callAction(middlewares) {
    ...
    return async function callAction(ctx, next) {
      ...
      let rt = await controller._callAction(ctx, middlewares, moduleName, ctrl, action, args, common)
