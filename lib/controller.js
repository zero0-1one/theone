    ...
    return async function callAction(ctx, next) {
      try {
        ...
        if (e instanceof RequestError) { //RequestError 属于外部调用错误，只记录 debug 日志
          theone.log.debug('Call action exception,  href:"%s", body:%s, msg:"%s"\nstack:"%s"', ctx.href, ctx.request.rawBody, e.message, e.stack)
        } else if (e instanceof Error) {
          theone.log.error('Call action exception,  href:"%s", body:%s, msg:"%s"\nstack:"%s"', ctx.href, ctx.request.rawBody, e.message, e.stack)
        } else {
          theone.log.error('Call action exception,  href:"%s", body:%s, msg:"%s"', ctx.href, ctx.request.rawBody, e.toString())
        }
