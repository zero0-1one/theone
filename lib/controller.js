  ...
  parsePath() {
    ...
    return async function parsePath(ctx, next) {
      try {
        ...
        if (ctx.app.errorHandlers.parsePath) await ctx.app.errorHandlers.parsePath(ctx, e)
        return
      }
      return next()
    }
  ...
  callAction(middlewares) {
    return async function callAction(ctx, next) {
      try {
        ...
        if (ctx.app.errorHandlers.callAction) await ctx.app.errorHandlers.callAction(ctx, e)
      }
    }
  }
}
