...
let controller = {
  ...
  inheritLastVersion(owner, suffix, lastVersion) {
    ...
    //继承静态方法或属性
    let lastOwnProNames = Object.getOwnPropertyNames(lastVersion.class)
    for (const name of lastOwnProNames) {
      if (!toUtil.hasOwnPropertySafe(owner, name)) {
        owner[name] = lastVersion.class[name]
      }
    }
    return owner
  },
  ...
  loadMiddlewares(dir, ctrl, middlewaresCfg = []) {
    ...
    for (const def of defs) {
      ...
      let args = typeof def == 'string' ? undefined : def.args
