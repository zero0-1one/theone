...
let controller = {
  ...
  getActionArgs(args, params) {
    ...
    for (let param of params) {
      ...
      if (arg === undefined) {
        if (param.conversion && !param.hasDefault) throw new TypeError(`参数 ${param.name} 必须是 ${param.type} 类型`)
      } else {
        if (param.conversion) {
          arg = param.conversion(arg)
          if (arg === undefined) throw new TypeError(`参数 ${param.name} 必须是 ${param.type} 类型`)
        }
      }
      sortArgs.push(arg)
    }
