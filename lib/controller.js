...
let controller = {
  extends(parent) {
    return class extends parent {
      ...
      failed(msg, data, status = 0) {
        return new Failed(msg, data, status)
      }
      ...
      isFailed(rt) {
        return rt instanceof Failed
      }

      isSucceed(rt) {
        return !this.isFailed(rt)
      }
  ...
  async callAction(ctx, actionMiddlewares, moduleName, ctrl, action, args, common = {}) {
    try {
      ...
      if (ctrlInst.isFailed(rt)) {
        rt = { status: rt.status, msg: rt.msg, data: rt.data }
      } else {
        rt = { status: 1, data: rt }
      }
      ctrlInst.destroy()
  ...
  async batchCall(ctx, actionMiddlewares, actions, common = {}) {
    ...
    for (let [moduleName, actionPath, args] of actions) {
      try {
        ...
        results.push({ status: -1, msg: e.toString() })
      }
    }
    return results
  },
    ...
    return async function callActions(ctx, next) {
      ...
      if (Array.isArray(actions) && actions.length > 0) {
        ...
        try {
          ...
          ctx.body = { status: -1, msg: e.toString() }
        }
      }
      await next()
    }
