...
module.exports = class Db {
  ...
  async _exec(type, sql, params = [], options = {}) {
    ...
    try {
      ...
      if (options.undefinedAsNull) params = params.map(v => v === undefined ? null : v)
      ...
      if (patternIndex == -1) {  //没有pattern
        ...
        if (options.undefinedAsNull) {
          if (is2d) {
            data = data.map(r => r.map(v => v === undefined ? null : v))
          } else {
            data = data.map(v => v === undefined ? null : v)
          }
        }
        ...
        let allParams = null
        let realSql = ''
        let maxRow = options.maxRow || 1000
        let results = []
        ...
        if (rest > 0) {
          ...
          results.concat((await this[_conn][rest > 10 ? 'query' : type](realSql, allParams))[0]) // rest 比较大的时候 固定使用 query 方法, 比保证不会有太多的 Prepared Statements
        }
        return results
      }
    } catch (e) {
