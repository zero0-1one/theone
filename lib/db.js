...
const _mustInTrans = Symbol('db must In Transaction')
...
module.exports = class Db {
  constructor(options, mustInTrans = false) {
    this[_options] = options
    this[_conn] = undefined //惰性初始化
    this[_begin] = false //是否开启了事务
    this[_mustInTrans] = mustInTrans
  }
  ...
  async query(sql, params, returnFields = false) {
    if (!this[_begin] && this[_mustInTrans]) {
  ...
  async batchReplace(table, fields, data, maxRow = 1000) {
    await this._batchOperation('REPLACE', table, fields, data, '', maxRow)
  }
  ...
  async batchInsert(table, fields, data, onDuplicate = '', maxRow = 1000) {
    ...
    await this._batchOperation('INSERT', table, fields, data, dupStr, maxRow)
  }

  async _batchOperation(op, table, fields, data, dupStr, maxRow) {
    let fieldsStr = ''
