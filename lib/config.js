...
module.exports = {
  DEF_CONFIG: {
    ...
    //有密集型计算需求的，可配置此项开启 worker_threads, 在 worker 线程中提供 具备与app（web）线程的 theone 环境
    //可通过数组配置多个 worker
    'worker': {
      //是否开启该worker线程
      'open': false,
      //该worker线程的名字 可在通过 theone.evn.workerName 获取，主线程值为 'main' 也可以通过 theone.app 是否存在来断。
      'name': '',
      //启动的实例个数, 可在通过 theone.evn.workerInstId 获取对应实例 id （编号从 0 开始）
      'instances': 1,
      //该worker线程的入库文件，可以是绝对或相对ROOT_DIR的路径
      'filename': '',
      //当worker线程退出后是否自动重启 默认：true
      'autorestart': true,
      // 心跳间隔单位 s， 默认：60秒。 当主线程超出 2倍 heartbeat 时间没有收到 worker 线程的心跳时会自动重启线程
      'heartbeat': 60,
      //是否开启 debug 模式方便调试，将开启新线程，而是在主线程加载入口文件。（注意！仅在 theone.env.DEBUG 开启时有效）
      'debug': false,
    },
  },
  ...
  load(configDir, envName) {
    ...
    this.adjustMultiConfig(config, 'worker', this.DEF_CONFIG['worker'])
    ...
    for (const module of config['modules']) {
      ...
      if (!module['database']) {
        ...
        // if(Array.isArray(module['database']))
