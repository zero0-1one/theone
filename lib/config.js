...
module.exports = {
  ...
  loadDir(obj, dir, ext) {
    ...
    for (let name of files) {
      ...
      if (stat.isFile()) {
        ...
        if (baseName == 'config') {
          obj = theone.util.deepAssign(obj, require(filePath))
        } else {
  ...
  adjustMultiConfig(obj, name, def) {
    ...
    if (Array.isArray(array)) {
      for (let i = 0; i < array.length; i++) {
        array[i] = theone.util.deepAssign(theone.util.deepCopy(def), array[i])
      }
    } else {
      obj[name] = [array]
    }
  ...
  load(configDir, configExt) {
    let config = theone.util.deepCopy(this.DEF_CONFIG)
...
theone.util.deepFreeze(module.exports.DEF_CONFIG)