...
module.exports = {
  DEF_CONFIG: {
    ...
    'jwt': {
      ...
      'options': { // jsonwebtoken.sign() 的第三个参数
    ...
    'cache': {
      ...
      'adapter': {
        ...
        'file': {
          ...
          // async gc(isExpired)
          'Adapter': require('./cacheAdapter/file'),   //file类型默认内置 fileCache
        ...
        'memory': {
          'Adapter': require('./cacheAdapter/memory'),   //file类型默认内置 fileCache
          'timeout': 60 * 60,
        }
      },
  ...
  loadConfig(filePath, envName = '') {
    ...
    if (envName && data.hasOwnProperty(envName)) {
  ...
  loadDir(obj, dir, envName, ext = '.js') {
    ...
    for (let name of files) {
      ...
      if (stat.isFile()) {
        ...
        toUtil.deepAssign(obj, { [name]: this.loadDir({}, filePath, ext) })
      }
    }
    return obj
  },
