...
module.exports = {
  DEF_CONFIG: {
    ...
    'modules': {
      ...
      //autoTransaction 为 true 此属性才有意义. 每个模块可以独立指定主数据库名(配置表中的 database.name 而非数据库名), 如果不指定则为 database[0].name
      'mainDb': undefined,
    },
  ...
  load(configDir, envName, configExt) {
    ...
    for (const module of config['modules']) {
      if (module['mainDb'] === undefined) module['mainDb'] = config['database'][0]['name']
    }
