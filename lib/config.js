...
const defConfig = {
  ...
  modules: {
    //模块名字 也是对应模块文件夹名
    'name': 'api',
    //相对 ROOT_DIR 的路径; 模块所在路径为 path.join(ROOT_DIR,  parentDir, name)
    'parentDir': './',
  ...
  log: {
    ...
    'type': 'stdout',
    //当 type 是 file 的时候有效, 
    'filename': theone.env.NAMESPACE + '.log',
    //当 type 是 file 的时候有效, 
...
module.exports = {
  ...
  loadDir(obj, dir, ext) {
    ...
    for (let name of files) {
      ...
      if (stat.isFile()) {
        ...
        if (baseName == 'config') {
          obj = theone.utility.deepAssign(obj, require(filePath))
        } else {
  ...
  adjustMultiConfig(obj, name, def) {
    ...
    if (Array.isArray(array)) {
      for (let i = 0; i < array.length; i++) {
        array[i] = theone.utility.deepAssign(theone.utility.deepCopy(def), array)
      }
    } else {
      obj[name] = [array]
    }
  ...
  load(configDir, configExt) {
    let config = theone.utility.deepCopy(defConfig)
    this.loadDir(config, configDir, configExt)

    this.adjustMultiConfig(config, 'database', defConfig.database)
    this.adjustMultiConfig(config, 'module', defConfig.module)
