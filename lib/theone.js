...
module.exports.callWorkerAction = async function (workerName, path, args) {
  ...
  taskWaitings[curTaskId]._timeout = setTimeout(() => {
    if (hasOwnPropertySafe(taskWaitings, taskId))
      taskWaitings[curTaskId].reject({ status: 'error', results: null, message: 'worker run timeout' })
  }, 60000)
...
module.exports.createWorker = function (environment, options, instanceId, onRestart) {
  ...
  parentPort.on('message', async (message) => {
    ...
    if (name == 'callWorkerAction') {
      try {
        ...
        parentPort.postMessage({ name, data: { status: 'error', results: err.stack, taskId: data.taskId } })
      }
    }
  })
  `
  ...
  worker.on('message', message => {
    ...
    if (name == 'heartbeat') {
      ...
      let { taskId, results } = data
      if (hasOwnPropertySafe(taskWaitings, taskId)) {
        taskWaitings[taskId].resolve(results)
