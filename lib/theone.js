...
module.exports._createEnv = async function (environment, workerName = 'main') {
...
module.exports.create = async function (environment = {}, init = () => {}) {
  ...
  if (this.config['worker']) {
    for (const options of this.config['worker']) {
      ...
      this.workers[options.name] = this.createWorker(environment, options, newWorker => {
        this.workers[options.name] = newWorker
      })
    }
  }
  initWaiting.resolve()
...
module.exports.createWorker = function (environment, options, onRestart) {
  let { name, filename, autorestart = true, heartbeat = 60 } = options
  let initScript = `
  const { parentPort } = require('worker_threads')
  const theone = require(${JSON.stringify(__filename)})
  const environment = ${JSON.stringify(environment)}
  theone._createEnv( environment, '${name}' ).then(
    require(${JSON.stringify(this.pathNormalize(filename))})
  )
  setInterval(()=>{
  ...
  worker.on('error', error => {
    this.log.error('[worker:%s] %s', name, error)
  })
  ...
  if (autorestart) {
    worker.on('exit', exitCode => {
      ...
      if (!this._shutdown && exitCode != 0 && !worker._isRestart) {
        let newWorker = this.createWorker(environment, options, onRestart)
        worker._isRestart = true
        this.log.error('[worker:%s] worker 线程自动重启', name)
...
module.exports.shutdown = async function () {
  ...
  for (const name in this.workers) {
    await this.workers[name].terminate()
